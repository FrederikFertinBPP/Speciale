...
from common_scripts.utils import cache_write, Trajectory, fields, load_trajectories, load_stats
...
class Agent: 
    ...
    def __init__(self, env: Env, writer=None):
        ...
        """
        self.env = env   
        self.writer = writer # Used to log training process of agents.
    ...
    def save(self):
        """ If the agent is trained, then this method exists to save the agent's learning progress to enable loading the agent in a future experiment. """
        pass
    
    def load(self):
        """ If the agent is trained, then this method exists to load the agent's learning progress. """
        pass

    def close(self):
        """ If the agent has some resources that need to be manually closed, this function can provide that utility. """
        pass
...
def _save_stats_and_trajectories(trajectories_, stats_, t_initial_, experiment_name_, verbose):
    t_stamp = get_unix_time()
    runtime = str(t_stamp - t_initial_).split('.')[0]
    t_stamp = str(t_stamp).split('.')[0]
    _filefolder = os.getcwd() + "/experiments/" + experiment_name_ + "-runtime-" + runtime + "-tstamp-" + str(t_stamp)
    file_path = _filefolder + "/trajectories.pkl"
    cache_write(trajectories_, file_path, verbose=verbose)
    file_path = _filefolder + "/stats.pkl"
    cache_write(stats_, file_path, verbose=verbose)
...
def train(env:Env,
    ...
    if agent is None:
        print("[train] No agent was specified. Using base Agent(env) (this agent selects actions at random)")
        agent = Agent(env)
    ...
    trajectories = []
    break_outer = False
    t_initial = get_unix_time()
    if continue_trajectories_and_stats:
        trajectories = load_trajectories(experiment_name=experiment_name)
        ...
        for i_episode in range(len(stats),num_episodes):
            ...
            for _ in itertools.count():
                ...
                if hasattr(env, 'step_with_hourly_model') and env.step_with_hourly_model:
                    obs_p, r, terminated, truncated, info_sp = env.step(a, agent.hourly_model)
                else:
                    obs_p, r, terminated, truncated, info_sp = env.step(a)

                done = terminated or truncated
                agent.train(obs, a, r, obs_p, done, info_s, info_sp)

                # Logging the training progress:
                trajectory.time.append(np.asarray(info_s['time'] if 'time' in info_s else steps)) #np.asarray(time))
                ...
                if done or steps >= max_steps:
                    trajectory.state.append(obs_p)
                    trajectory.env_info.append(info_sp)
                    trajectory.time.append(np.asarray(info_sp.get('time', None) if 'time' in info_s else steps))
            ...
            if save_every is not None:
                if i_episode % save_every == save_every-1:
                    if experiment_name:
                        _save_stats_and_trajectories(trajectories, stats, t_initial, experiment_name, verbose)
    ...
    if agent.writer is not None:
        agent.writer.flush()
    if save_agent:
        agent_folder = os.getcwd() + f"/models/rl_models/{experiment_name}"
