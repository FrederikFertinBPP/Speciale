...
from common_scripts.utils import cache_write, Trajectory, fields
...
def train(env,
        ...
        for i_episode in range(num_episodes):
            ...
            for _ in itertools.count():
                ...
                if verbose and k%3==2:
                    print(f"Episode: {i_episode}, iter: {k}")
                a = agent.pi(s, k, info_s)
                k += 1
    ...
    if experiment_name:
        ...
        runtime = str(t_stamp - t_initial).split('.')[0]
        t_stamp = str(t_stamp).split('.')[0]
        _filefolder = os.getcwd() + "/experiments/" + experiment_name + "_runtime:" + runtime + "_tstamp:" + str(t_stamp)
        file_path = _filefolder + "/trajectories.pkl"
        cache_write(trajectories, file_path, verbose=verbose)
        if hasattr(agent, 'save_logbook_as_csv'):
            file_path = _filefolder + "/agent_logbook.csv"
            agent.save_logbook_as_csv(file_path)
    # sys.stderr.flush()
