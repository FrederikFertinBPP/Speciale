...
from common_scripts.utils import Flattener, cache_exists, cache_read, cache_write
from gymnasium.spaces import flatten

from model_scripts.torch_networks import TorchActorNetwork, TorchCriticNetwork, TorchActorWarmstarter, TorchActorNetworkSteering
...
class DDPGAgent(Agent):
    def __init__(self,
                 ...
                 replay_buffer_minreplay=366, # Minimum number of environment interactions stored in the memory (buffer) before training is done.
                 ...
                 theta=0.9,               # Autoregressive term of Ornstein-Uhlenbeck process
                 dt=1,                    # Time step of problem (relevant for Ornstein-Uhlenbeck process (continuous AR process))
    ...
    def _get_action_noise(self):
        ...
        self.x += self.dt * (self.theta * (self.mu - self.x) + self.sigma * np.random.randn(self.num_actions))
        return self.x
    ...
    def pi(self, obs, k, info_s=None):
        ...
        chosen_actions = self.actor(obs)

        noise_scalars = np.clip(np.abs(chosen_actions + noise), 1, np.inf) * eps_ # Log noise to encourage more exploration at high value, but also a significant amount at low.
        actions = chosen_actions + noise_scalars * noise  # Add noise for exploration
        return actions.reshape(self.env.action_space.shape)


class NN_DDPGAgent(DDPGAgent):
    def _create_actor_critic(self, hidden_size, critic_warmstarter=None, actor_warmstarter=None, initial_guess:np.ndarray|None = None, **kwargs):
        ...
        self.critic         = TorchCriticNetwork(self.env, trainable=True, learning_rate=self.alpha, hidden=hidden_size, writer=self.writer)
        if critic_warmstarter is not None:
            self.critic.update_Phi(critic_warmstarter, tau=1)
        self.critic_target  = TorchCriticNetwork(self.env, trainable=False, learning_rate=self.alpha, hidden=hidden_size)  # Target network
        self.critic_target.update_Phi(self.critic, tau=1.0)  # Initialize target network to match critic
        self.actor          = TorchActorNetworkSteering(self.env, trainable=True, learning_rate=self.alpha, hidden=hidden_size, writer=self.writer)
        if initial_guess is not None:
            assert len(initial_guess) == self.num_actions, f"The initial guess should be a vector with the size of the number of actions ({self.num_actions})."
        ...
        self.actor_target   = TorchActorNetworkSteering(self.env, trainable=False, learning_rate=self.alpha, hidden=hidden_size)  # Target network
    ...
    def save(self, path):
        ...
        self.memory.steps = self.steps
        self.memory.episodes = self.episodes
        self.memory.save(path + "/memory.pkl")
    ...
    def load(self, path):
        ...
        if cache_exists(path + "/memory.pkl"):
            self.memory = cache_read(path + "/memory.pkl")
            self.steps = self.memory.steps
            self.episodes = self.memory.episodes

    def train(self, obs, a, r, obs_p, done=False, info_s=None, info_sp=None):
        obs = flatten(self.env.observation_space, obs)
        obs_p = flatten(self.env.observation_space, obs_p)
        a = list(Flattener().flatten(a).values())
...
class WarmStartDDPGAgent(NN_DDPGAgent):
