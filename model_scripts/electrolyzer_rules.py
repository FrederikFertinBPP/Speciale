...
# class ElectrolyzerRules:
#     def piecewise_linear_efficiency(self, )
...
#         b.ec_constraint = pyo.Constraint(self.model.T, self.model.PWE, rule=ec_rule)
import numpy as np
...
def plot_el_curves(El, config_dict):
    ...
    if config_dict["eff_type"]==2: #HYP_L  
        ...
        for jj in range(0, N_s):
            p_vec=np.linspace(El["p_val"][jj],El["p_val"][jj+1])
            p_vec=np.linspace(El["P_min"]*El["El_cap"],El["El_cap"])
            plt.figure(fig_eff)
...
if __name__ == "__main__":
    El = {  'El_cap_tot': 1, # Total electrolyzer capacity [MW]
            'El_cap':1, # Electrolyzer unit (i.e., stack) capacity [MW]
            'N_u': 1, # number of units
            'P_min':0.15, # % minimum load
            'P_sb': 0.01, # % Power consumption in stand-by state
            'C_cold': 50, # Cold startup cost  [EUR/MW]
        }
    segments = 5
    P_eta_max = p_eta_max_fun(El) # around 28% of maximum power
    p_val = [El["P_min"],0.215, P_eta_max,  0.37,	0.5,	0.7,  1]# np.concatenate((np.linspace(El['P_min'], P_eta_max, num=3), np.linspace(P_eta_max, El['El_cap'], num=segments-1)[1:]))
    El.update({'P_eta_max': P_eta_max})
    El.update({"p_val": p_val})
    config_dict = {"eff_type": 2}
    initialize_electrolyzer(El, config_dict)
    
    plot_el_curves(El, config_dict)

    for key, value in El.items():
        if isinstance(value, np.ndarray):
            print(f"{key}: {value.tolist()}")
            El.update({key: value.tolist()})
    import json
    with open('setup_files/electrolyzer_technicals.json', 'w') as f:
        json.dump(El, f)
