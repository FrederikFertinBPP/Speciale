...
class TorchNetwork(nn.Module,DQNNetwork):
    def __init__(self, env, trainable=True, learning_rate=0.001, hidden=30, writer=None):
        ...
        self.num_states = np.prod(env.state_space.shape)
...
class TorchActorNetwork(TorchNetwork):
    def __init__(self, env, trainable=True, learning_rate=0.001, hidden=30, context_size=0, writer=None):
        super().__init__(env, trainable, learning_rate, hidden, writer)
        self.num_features = self.num_states + context_size
        self.build_model_()
        if trainable:
...
class TorchCriticNetwork(TorchNetwork):
    def __init__(self, env, trainable=True, learning_rate=0.001, hidden=30, context_size=0, writer=None):
        super().__init__(env, trainable, learning_rate, hidden, writer)
        self.num_content = context_size
        self.num_features = self.num_states + self.num_actions + self.num_content  # State + Action + Context
