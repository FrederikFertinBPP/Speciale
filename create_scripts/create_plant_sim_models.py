...
from common_scripts.utils import cache_write, log_transform
import pandas as pd
import numpy as np
...
def get_hourly_df(resource='pv'):
    df = pd.read_csv(f"historical_data/renewablesninja_{resource}_PT.csv", skiprows=3)
...
end     = pd.Timestamp('20201031', tz='UTC')
...
year_month_index = df_wind.index.tz_localize(None).to_period('M')
df_wind_system = data_object.data.loc[df_wind.index, 'wind'] / year_month_index.map(data_object.caps['wind'])
df_solar_system = data_object.data.loc[df_solar.index, 'solar'] / year_month_index.map(data_object.caps['solar'])
