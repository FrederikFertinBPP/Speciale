...
class RFPEnv(gym.Env):
    ...
    def __init__(self, rfp:RenewableFuelPlant, normalize:bool=True):
        ...
        self.action_space = gym.spaces.Box(low = np.asarray([[-self.hpp.get_component("GCP").parameters['capacity'], 0, 0, 0]]*self.horizon,
                                                             dtype = np.float64),
                                           high = np.asarray([[self.hpp.get_component("GCP").parameters['capacity'],
                                                              ...
                                                              self.hpp.get_component("Haber-Bosch Plant").parameters['capacity']]] * self.horizon,
                                                              dtype = np.float64),
    ...
    def realize_stochasticity(self, steps=1):
        ...
        """
        p_wind = np.random.uniform(0, self.hpp.get_component("WindPower").parameters["capacity"], size=steps)  # Simulated wind power in MW
        p_solar = np.random.uniform(0, self.hpp.get_component("SolarPower").parameters["capacity"], size=steps)  # Simulated solar power in MW
