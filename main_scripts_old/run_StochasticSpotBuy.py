...
from model_scripts_old.agent_hierarchical_heuristic import SpotBuyStochasticHA
...
def main():
    ...
    rfp = create_rfp()
    rfp.get_contract('Ammonia1').parameters['volume'] = rfp.get_component("Haber-Bosch Plant").parameters.get('capacity') * 8760 / 2 # 50% capacity contracted
    env = SpotBuyRFPEnv(rfp=rfp, forecaster=forecaster, decision_horizon=decision_horizon, planning_horizon=planning_horizon)
    
    """ Set experiment indicator """
    experiment = "testStochasticSpotbuy_"
    ...
    agent = SpotBuyStochasticHA(env=env, guideline=guideline, solver='gurobi_persistent', n_scenarios=10)
...
if __name__ == '__main__':
    cProfile.run("main()", "run_profiles/run_StochasticSpotBuy.prof")